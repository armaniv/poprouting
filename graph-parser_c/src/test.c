...
int main(int argc, char** argv)
{
  ...
  char * file_content=read_file_content("topology.json");
  ...
  */
  ...
  if(heuristic==1)
    fptr = fopen("Euristica modif.dat", "w");
  else
    fptr = fopen("Brandes.dat", "w");
  ...
  for(int i=0; i<bc_degree_map->size; i++)
    fprintf(fptr,"%s %1.50f \n",bc_degree_map->map[i].id,bc_degree_map->map[i].bc);
